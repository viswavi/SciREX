...
parser.add_argument("--compute-mean-average-precision", action='store_true', help="Whether or not to compute mean average precision for retrieval metrics (takes about 3 minutes)")
parser.add_argument("--choose-dev-thresholds", action='store_true', help="If false, use fixed threshold. If true, search for best threshold on dev set performance - takes about 20 minutes.")
...
def compute_relations_metrics(gold_data, predicted_ner, predicted_salient_clusters, predicted_relations, predicted_cluster_to_gold_cluster_map, thresh=None, n=4, average='binary', compute_mean_average_precision=False):
    ...
    for types in combinations(used_entities, n):
        for doc in gold_data:
            ...
            cache_tuple = doc.get(doc_id, {}).get(tuple(types), None)
            if doc_id is not None and cache_tuple is not None:
                relations_seen, relations_with_scores = cache_tuple
            else:
                relations_seen = set()
                ...
                if doc["doc_id"] not in cache_2:
                    cache_2[doc["doc_id"]] = {}
                cache_2[doc["doc_id"]][tuple(types)] = relations_seen, relations_with_scores
...
def main(args):
    ...
    for n in [2, 4]:
        retrieval_metrics, classification_metrics, mean_average_precision, = compute_relations_metrics(gold_data,
                                                ...
                                                thresh=None,
                                                average='macro',
                                                compute_mean_average_precision=args.compute_mean_average_precision)
    ...
    if args.choose_dev_thresholds:
        ...
        print(f"Best threshold is {round(best_threshold, 4)}, with dev-set classification metric (n=4) value of {round(best_f1, 4)}")

        for n in [2, 4]:
            print(f"At threshold {str(best_threshold)}:")
